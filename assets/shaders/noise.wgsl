...
fn noise(pos: vec3<f32>) -> f32{
    ...
    var d : vec3<f32> = pos - a;
    d = d * d * (3.0 - 2.0 * d);

    var b : vec4<f32>= a.xxyy + vec4<f32>(0.0, 1.0, 0.0, 1.0);
    var k1: vec4<f32> = perm(b.xyxy);
    var k2: vec4<f32> = perm(k1.xyxy + b.zzww);

    var c : vec4<f32>= k2 + a.zzzz;
    var k3: vec4<f32> = perm(c);
    var k4: vec4<f32> = perm(c + 1.0);

    var o1: vec4<f32> = fract(k3 * (1.0 / 41.0));
    var o2: vec4<f32> = fract(k4 * (1.0 / 41.0));

    var o3: vec4<f32> = o2 * d.z + o1 * (1.0 - d.z);
    var o4: vec2<f32> = o3.yw * d.x + o3.xz * (1.0 - d.x);
